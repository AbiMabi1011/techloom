...
export default function Hero({ onExplore, onCategorySelect, categories = [], products = [] }) {
  ...
  return (
    <div className="relative overflow-hidden bg-gradient-to-b from-[#0F1420] via-[#0B0F17] to-[#0B0F17] border-b border-white/10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 pb-12 sm:pt-8 sm:pb-16 lg:pt-10 lg:pb-20 relative z-10">
        
        {/* Main Banner Interactive Carousel */}
        <div 
          className="relative rounded-2xl sm:rounded-3xl overflow-hidden border border-white/10 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.7)] bg-[#121722]"
          onMouseEnter={() => setAutoplay(false)}
          onMouseLeave={() => setAutoplay(true)}
        >
          {/* Slide Background Image with Smooth Crossfade */}
          <div className="relative min-h-[460px] sm:min-h-[500px] lg:min-h-[550px] flex items-center">
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-[#070A0F]/95 via-[#070A0F]/90 sm:via-[#070A0F]/85 md:via-[#070A0F]/75 to-transparent"></div>
            <div className="absolute inset-0 bg-gradient-to-t from-[#070A0F] via-transparent to-[#070A0F]/40"></div>

            {/* Slide Text Content */}
            <div className="relative z-10 max-w-2xl p-5 sm:p-10 lg:p-16 space-y-4 sm:space-y-6">
              
              {/* Promotional Pill */}
              <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1 sm:py-1.5 rounded-full backdrop-blur-md border border-white/15 bg-black/50 shadow-sm max-w-full">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping shrink-0"></span>
                <span className="text-[10px] sm:text-xs font-bold tracking-wider uppercase text-slate-200 truncate">
                  {slide.tag}
                </span>
                <span className="text-slate-600 shrink-0">|</span>
                <span className="text-[10px] sm:text-xs text-amber-300 font-semibold shrink-0">{slide.discount}</span>
              </div>

              {/* Headline */}
              <h1 className="font-display text-2xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tracking-tight leading-[1.15] sm:leading-[1.1]">
                {slide.titleMain} <br />
              ...
              {/* Sub-copy */}
              <p className="text-slate-300 text-xs sm:text-sm md:text-base leading-relaxed max-w-lg line-clamp-3 sm:line-clamp-none">
                {slide.desc}
              </p>
              ...
              <div className="pt-2 flex flex-wrap items-center gap-3 sm:gap-4">
                ...
                >
                  ...
                  <svg className="w-4 h-4 font-bold shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3" />
                  </svg>
                </button>

                <div className="w-full sm:w-auto justify-center flex items-center gap-2 sm:gap-3 px-3.5 sm:px-5 py-2.5 sm:py-3.5 rounded-xl bg-black/60 border border-white/10 backdrop-blur-md text-white text-[11px] sm:text-xs font-medium">
                  <span className="text-amber-400 font-mono font-bold text-[10px] sm:text-xs px-2 py-0.5 rounded bg-amber-400/10 border border-amber-400/20">
                    CODE: {slide.promoCode}
                  </span>
                  <span className="text-slate-500">|</span>
                  <span className="text-slate-300 text-[10px] sm:text-xs">5-Min Stock Lock</span>
                </div>
              </div>
            </div>

            {/* Floating Top Right Tag - Hidden on mobile to avoid covering text */}
            <div className="hidden md:flex absolute top-8 right-8 backdrop-blur-xl bg-black/70 border border-white/15 px-4 py-2.5 rounded-2xl items-center gap-3 shadow-xl">
            ...
            <div className="absolute bottom-4 sm:bottom-6 right-4 sm:right-8 z-20 flex items-center gap-1.5 sm:gap-2">
              <button
                ...
                className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-black/60 hover:bg-black/80 border border-white/15 text-white flex items-center justify-center transition-all backdrop-blur-md active:scale-95 text-xs sm:text-sm"
            ...
            <div className="absolute bottom-4 sm:bottom-6 left-4 sm:left-1/2 sm:-translate-x-1/2 z-20 flex items-center gap-1.5 sm:gap-2">
              {dynamicSlides.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setCurrentSlide(i)}
                  className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 ${
                    i === currentSlide ? 'w-6 sm:w-8 bg-amber-400' : 'w-1.5 sm:w-2 bg-white/30 hover:bg-white/60'
                  }`}
                />
              ))}
            </div>
        ...
        <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 px-1">
          <div className="flex items-center gap-2 sm:gap-2.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none">
            <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 shrink-0">
              Trending:
            </span>
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              {displayCategories.map((cat) => (
                <button
                  ...
                  className="text-[11px] sm:text-xs px-3 py-1 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/25 text-slate-300 hover:text-white transition-all whitespace-nowrap active:scale-95"
                >
                  {cat}
                </button>
              ))}
          ...
          <div 
            className="flex items-center gap-1.5 text-xs text-amber-400 font-semibold cursor-pointer hover:underline self-end sm:self-auto"
            onClick={onExplore}
          >
